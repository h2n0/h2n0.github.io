...
Game.Screen.playScreen = {
    ...
    enter: function() {
        ...
        map.getEngine().start();
        this._player.addItem(Game.ItemRepository.create('rock'));
    },
    ...
    render: function(display) {
        ...
        var hungerPerc = this._player.getHungerPercent()+"% ";
        var hString = hungerPerc + hungerState
        display.drawText(screenWidth - hString.length - 1, screenHeight, hString);
    },
...
Game.Screen.winScreen = {
    ...
    render: function(display) {
        var lines = ["---Well Done!---","You succesfuly beat the zombie in the cave","But the villages have another task they","would like you to complete","Press [Enter] to continue on your quest"];
        for(var i = 0; i < lines.length; i++){
            if(i == lines.length - 1){
                display.drawText(Game.getScreenWidth() / 2 - lines[i].length / 2, 10 + i + 1 ,lines[i]);
            }else{
...
Game.Screen.eatScreen = new Game.Screen.ItemListScreen({
    ...
    ok: function(selectedItems) {
        ...
        if (!item.hasRemainingConsumptions()) {
            ...
            //Game.sendMessage(this._player, "You eat part of the %s.", [item.describeThe()]);
        }
        return true;
    }
});
    ...
    ok: function(selectedItems) {
        ...
        if (keys.length > 0) {
            ...
            if(item.getName() == 'rock'){
                ...
                if(chance >= (1 - 0.01)){
                    this._player.giveExperience(100);
                    this._player.removeItem(item);
                    Game.sendMessage(this._player,"You feel slightly more knowlageable");
                }else if(chance <= (1 - 0.8)){
                    Game.sendMessage(this._player,"You over examined the "+item.getName());
                    this._player.removeItem(item);
                }
            }
        }
...
Game.Screen.gainStatScreen = {
    ...
    render: function(display) {
        var letters = 'abcdefghijklmnopqrstuvwxyz';
        display.drawText(1, 1, 'Choose a stat to increase: ');

        // Iterate through each of our options
        for (var i = 0; i < this._options.length; i++) {
            display.drawText(1, 3 + i, letters.substring(i, i + 1) + ' - ' + this._options[i][0]);
        }

        // Render remaining stat points
        display.drawText(1, 5 + this._options.length,
